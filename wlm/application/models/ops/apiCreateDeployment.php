...
class apiCreateDeployment extends apiIndex {
		...
		try{
			...
			if ((isset($submit)) && (isset($testfile))) {
				...
				if (isset($appid) && !empty($appid)) {
					$body["AppId"] = str_replace("%appid%", $appid, $body["AppId"]);
				}else{
					$body["AppId"] = str_replace("%appid%", "", $body["AppId"]);
				}
				if (isset($instanceid) && !empty($instanceid)) {
					$body["InstanceIds"] = array(
						"$instanceid"
					);
				}
	...
	private function createForm($request) {
		...
		$view->setSearchPattern("appinfo");
		$view->addColumnItems("stackname", $stackname);
		$view->addColumnItems("stackid", "$stackid");
		$view->addColumnItems("instanceid", isset($instanceid) ? $instanceid : "");
		$view->addColumnItems("appid", isset($appid) ? $appid : "");
		...
		$view->addColumnItems("stackname", "$stackname");
		$view->addColumnItems("stackid", "$stackid");
		$view->addColumnItems("layerid", isset($layerid) ? $layerid : "");
		$view->addColumnItems("instanceid", isset($instanceid) ? $instanceid : "");
		$view->addColumnItems("appid", isset($appid) ? $appid : "");
		$view->addColumnItems("deployid", isset($deployid) ? $deployid : "");
